...
本章介绍 FreeRTOS 官方分发包的目录组织与工程集成方法，包含：
...
## 2.2 理解 FreeRTOS 分发结构

### 2.2.1 什么是 Port

一个 **port** 指“某编译器 + 某处理器架构”的 FreeRTOS 适配组合。

### 2.2.2 构建方式

FreeRTOS 以源文件形式集成到用户工程：

- 通用内核文件
- 与 port 相关文件

官方为每个 port 提供 Demo，便于开箱构建与验证。

### 2.2.3 FreeRTOSConfig.h

`FreeRTOSConfig.h` 是应用级内核配置头。建议：

- 在用户代码中包含 `FreeRTOS.h`，不要直接包含 `FreeRTOSConfig.h`。
- 从目标 port 的 Demo 配置出发再按需裁剪。

### 2.2.4 获取分发包

最新版本：<https://github.com/FreeRTOS/FreeRTOS/releases/latest>
...
- `event_groups.c`（按需）
- `stream_buffer.c`（按需）
- `croutine.c`（历史兼容，不建议新设计）

### 2.2.6 Port 相关源文件

位于 `FreeRTOS/Source/portable`，按编译器与架构分层。

堆实现位于 `portable/MemMang`。若启用动态分配，应仅选择一个 `heap_n.c`（或自定义实现）。
...
- `FreeRTOS.h`
- 对应模块头（`task.h`、`queue.h`、`semphr.h`、`timers.h`、`event_groups.h` 等）
...
1. 先构建并运行原 Demo；
2. 移除示例业务任务；
3. 保留硬件初始化与 `vTaskStartScheduler()`；
4. 再按功能逐步添加应用任务。
...
- 先建立可运行裸机工程；
- 再加入 FreeRTOS 核心 + port 文件；
- 拷贝并调整 `FreeRTOSConfig.h`；
- 同步 Demo 的编译器与中断配置。
...
- `TickType_t`：滴答计数与时间参数类型。
- `BaseType_t`：架构最优基础整型。

### 2.5.2 命名风格（摘要）

- 变量名前缀表达类型；
- 函数名前缀体现返回类型与模块来源（如 `vTask...`、`xQueue...`）；
- 私有函数常用 `prv` 前缀。
