...
[第 2.5 节：数据类型与编码风格指南](ch02-FreeRTOS%20内核分发包.md)
描述了所使用的数据类型和命名约定。
...
- `uxPriority`
  ...
  [第 4.5 节](#45)描述了用户定义常量 `configMAX_PRIORITIES`。
...
- 返回值
  ...
  - `pdFAIL`
    ...
    表示堆内存不足，无法创建任务。
    [第 3 章](ch03-堆内存管理.md)提供了更多堆内存管理信息。
...
[第 4.12 节 调度算法](#412)介绍了一个可选特性“时间片（time slicing）”。
